        ...
        <div style={{ maxWidth:1140, margin:'0 auto' }}>
          <div className="pk-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:72, alignItems:'center' }}>
            <div ref={textRef} style={{ opacity:textVisible?1:0, transform:textVisible?'translateX(0)':'translateX(-40px)', transition:'opacity 0.8s ease, transform 0.8s ease' }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,3vw,38px)', fontWeight:800, color:'#1A1A1A', lineHeight:1.2, letterSpacing:'-0.8px', marginBottom:16 }}>A System Built for Service Operations</h2>
              <p style={{ color:'#666', fontSize:16, lineHeight:1.85, marginBottom:32 }}>Projekco organizes your business into a structured system — from job scheduling to team coordination and service reporting, everything centralized in one platform.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                {outcomes.map(({ num, label, desc }, i) => (
                  <div key={num} className="pk-outcome" style={{ display:'flex', gap:16, alignItems:'flex-start', opacity:textVisible?1:0, transform:textVisible?'translateY(0)':'translateY(20px)', transition:`opacity 0.5s ${0.15+i*0.12}s ease, transform 0.5s ${0.15+i*0.12}s ease` }}>
