        ...
        <div style={{ maxWidth:1140, margin:'0 auto' }}>
          <div className="pk-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:72, alignItems:'center' }}>
            <div ref={textRef} style={{ opacity:textVisible?1:0, transform:textVisible?'translateX(0)':'translateX(-40px)', transition:'opacity 0.8s ease, transform 0.8s ease' }}>
              ...
              <p style={{ color:'#666', fontSize:16, lineHeight:1.85, marginBottom:28 }}>Whether you operate a small team or manage multiple service crews, Projekco provides the tools to keep everything organized and running smoothly.</p>
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:32 }}>
                {industries.map(({ icon, label }, i) => (
                  <div key={label} className="pk-industry-row" style={{ display:'flex', alignItems:'center', gap:14, background:'#fff', border:'1px solid #E8DFD0', borderRadius:12, padding:'13px 18px', opacity:textVisible?1:0, transform:textVisible?'translateY(0)':'translateY(16px)', transition:`opacity 0.5s ${0.1+i*0.08}s ease, transform 0.5s ${0.1+i*0.08}s ease` }}>
              ...
              <a href="#demo" style={{ display:'inline-block', padding:'13px 30px', borderRadius:10, background:'#2D2D2D', color:'#fff', textDecoration:'none', fontWeight:600, fontSize:15 }}>See How It Works</a>
            </div>
