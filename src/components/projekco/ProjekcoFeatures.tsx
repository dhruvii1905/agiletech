...
const ProjekcoFeatures = () => {
  ...
  return (
    <>
      <style>{`
        @keyframes pk-fadeUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}
        ...
        <div style={{ maxWidth:1140, margin:'0 auto' }}>
          ...
          <div ref={gridRef} className="pk-feat-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:24 }}>
            {features.map(({ img, title, desc, tags }, i) => (
              <div key={title} className="pk-feat-card" style={{
                background:'#FAFAF8', border:'1px solid #EDE8DF', borderRadius:16, overflow:'hidden',
