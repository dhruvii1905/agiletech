  ...
  return (
    <>
      <style>{`
        ...
        .pk-price-btn { transition: opacity 0.2s, transform 0.2s; display:block; }
        .pk-price-btn:hover { opacity:0.85!important; transform:translateY(-1px); }
        @media(max-width:900px){.pk-price-grid{grid-template-columns:1fr!important;max-width:400px;margin:0 auto}}
      `}</style>
      <section id="pricing" style={{ background:'#fff', padding:'100px 24px', borderBottom:'1px solid #E8DFD0' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <div ref={headRef} style={{ textAlign:'center', marginBottom:60, opacity:headVisible?1:0, transform:headVisible?'translateY(0)':'translateY(30px)', transition:'opacity 0.7s ease, transform 0.7s ease' }}>
            ...
            <p style={{ color:'#666', fontSize:16, maxWidth:440, margin:'0 auto' }}>Choose the plan that fits your team size and operational needs.</p>
          </div>
          <div ref={gridRef} className="pk-price-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:24, alignItems:'start' }}>
            {plans.map(({ name, sub, features, highlight }, i) => (
              <div key={name} className="pk-price-card" style={{ background:highlight?'#2D2D2D':'#FAFAF8', border:highlight?'none':'1px solid #EDE8DF', borderRadius:18, padding:'38px 30px', position:'relative', boxShadow:highlight?'0 12px 40px rgba(0,0,0,0.15)':'0 2px 8px rgba(0,0,0,0.04)', opacity:gridVisible?1:0, transform:gridVisible?'translateY(0)':'translateY(40px)', transition:`opacity 0.6s ${i*0.12}s ease, transform 0.6s ${i*0.12}s ease` }}>
                {highlight && <div style={{ position:'absolute', top:-13, left:'50%', transform:'translateX(-50%)', background:'#8B7355', borderRadius:100, padding:'4px 18px', color:'#fff', fontSize:12, fontWeight:700, whiteSpace:'nowrap' }}>Most Popular</div>}
                ...
                <a href="#demo" className="pk-price-btn" style={{ textAlign:'center', padding:'13px 0', borderRadius:10, background:highlight?'#8B7355':'#2D2D2D', color:'#fff', textDecoration:'none', fontWeight:600, fontSize:15 }}>Get Started</a>
              </div>
            ))}
          </div>
        </div>
