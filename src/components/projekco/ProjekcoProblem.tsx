...
const ProjekcoProblem = () => {
  ...
  return (
    <>
      <style>{`
        @keyframes pk-slideRight { from{opacity:0;transform:translateX(-40px)}to{opacity:1;transform:translateX(0)} }
        @keyframes pk-slideLeft  { from{opacity:0;transform:translateX(40px)} to{opacity:1;transform:translateX(0)} }
        @keyframes pk-fadeUp     { from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)} }
          ...
          <div className="pk-grid-2" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:72, alignItems:'center' }}>
            ...
            <div ref={textRef} style={{ opacity: textVisible?1:0, transform: textVisible?'translateX(0)':'translateX(40px)', transition:'opacity 0.8s ease, transform 0.8s ease' }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,3vw,38px)', fontWeight:800, color:'#1A1A1A', lineHeight:1.2, letterSpacing:'-0.8px', marginBottom:16 }}>
                Service Businesses Often Run on Chaos
              </h2>
              <p style={{ color:'#666', fontSize:16, lineHeight:1.85, marginBottom:24 }}>
                Many service companies operate with scattered tools and manual coordination.
                Without a system, growth becomes difficult — and mistakes become costly.
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {problems.map(({ icon, text }, i) => (
                  <div key={text} className="pk-prob-card" style={{
                    display:'flex', alignItems:'flex-start', gap:14,
                    background:'#FAFAF8', border:'1px solid #EDE8DF', borderRadius:12, padding:'14px 18px',
                    opacity: textVisible?1:0, transform: textVisible?'translateY(0)':'translateY(16px)',
                    transition: `opacity 0.5s ${0.1 + i * 0.1}s ease, transform 0.5s ${0.1 + i * 0.1}s ease`,
                  }}>
